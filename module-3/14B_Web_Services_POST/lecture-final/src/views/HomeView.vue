  ...
  methods: {
    ...
    saveNewBoard() {
      ...
      boardService.addBoard(this.newBoard).then(response => {
        ...
        if (error.response) {
          this.$store.commit('SET_NOTIFICATION',
            "Error adding new board. Response received was '" + error.response.statusText + "'.");
        } else if (error.request) {
          this.$store.commit('SET_NOTIFICATION',
            "Error adding new board. Server could not be reached.");
        } else {
          this.$store.commit('SET_NOTIFICATION',
            "Error adding new board. Request could not be created.");
        }
        this.isLoading = false;
      });
    },
